...
import com.dysen.library.SyncHorizontalScrollView;
...
public class MoreTypeActivity extends AppCompatActivity {
    ...
    private void initView() {
        ...
        mAdapter = new SuperRecyclerAdapter<Person>(this, mList) {
            @Override
            public void convert(SuperRecyclerHolder holder, final Person person, int layoutType,
                                final int position) {
                TextView vv = (TextView) holder.getItemView().findViewById(R.id.vv);
                vv.setText("==========================================");
                //只有一种布局，不使用layoutType来区分type了
                holder//
                        .setText(Arrays.asList(new Integer[]{R.id.tv_name, R.id.tv_addr, R.id.tv_age, R.id.tv_phone}),
                                Arrays.asList(new String[]{person.getName(), person.getAddr(), String.valueOf(person.getAge()), String.valueOf(person.getNumber())}), "--")
            ...
            @Override
            public int getLayoutAsViewType(Person s, int position) {
...
//        mRv.setLayoutManager(gridLayoutManager);
    ...
    private void initData() {
        for (int i = 1; i <= 100; i++) {
            if (i % 10 == 0)
                mList.add(new Person("", null, -1, -1));
            mList.add(new Person("张三" + i, "中国北京的长安街" + i + "号", i + 10, i));
        }
